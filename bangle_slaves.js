...
// !!! CHANGE THIS NAME FOR EACH OF THE 3 FOLLOWERS BEFORE FLASHING !!!
const DEVICE_NAME = "LEFT HAND"; 
...
function drawReadyScreen() {
  ...
  g.setFont("Vector",34);
  g.setColor(0,1,0).drawString("READY",120,95);
  
  g.setColor(1,1,1);
  g.setFont("Vector",18);
  g.drawString("Waiting for sync...",120,145);
  g.setFont("Vector",14);
  g.drawString("BTN1 = Power Off Device",120,185);
  g.drawString("BTN2 = Local Start Override",120,210);
}
...
// Wireless Sync Listener Engine
function startWirelessSyncListener() {
  g.setFont("Vector",14).setColor(0,1,1);
  g.drawString("☤ Sync Armed", 120, 10);

  // Cleaned filter parameters to parse raw Bluetooth data arrays
  NRF.setScan(function(d) {
    if (d.manufacturer === 0x0590 && d.manufacturerData && d.manufacturerData[0] === 0xAA && d.manufacturerData[1] === 0xBB) {
      NRF.setScan(); 
      Bangle.buzz(400); 
      scheduleRecording(5); 
    }
...
// Wireless Sync Stop Listener Engine (Listens while recording)
function startWirelessStopListener() {
  NRF.setScan(function(d) {
    if (d.manufacturer === 0x0590 && d.manufacturerData && d.manufacturerData[0] === 0xCC && d.manufacturerData[1] === 0xDD) {
      NRF.setScan(); 
      recStop(); 
    }
  });
...
function drawRecordingScreen(timeStr){
  ...
  g.setFont("Vector",14);
  g.drawString("BTN3 = Stop Recording",120,225); 
}
...
function startLogging() {
    ...
    // Followers begin scanning for remote STOP commands
    startWirelessStopListener();
...
function recStop() { "ram";
    ...
    NRF.setScan(); // Kill follower remote stop scanning
...
function startActivate() {
  ...
  myStatus[9] = E.getBattery(); myStatus[10] = E.getTemperature(); 
  g.clear();
  ...
  setWatch(function() {
    if (recordState == STATE_FINISHED) {
      setupStandardReadyState();
    } else if (recordState == STATE_READY) {
      NRF.setScan(); // Stop scanning while displaying confirm screen
  ...
  // BTN2 listener (Local start override)
  setWatch(function(){
      if(recordState == STATE_READY){
          NRF.setScan(); 
          scheduleRecording(5);
  ...
  // BTN3 listener (Stop recording OR Cancel Shutdown Confirm)
  setWatch(function() {
    if (recordState == STATE_RECORDING) {
      recStop();
    } else if (recordState == STATE_SHUTDOWN_CONFIRM) {
      setupStandardReadyState(); // Cancel and go back to READY
    }
  ...
  if (fList.length > 0) {
    ...
    setWatch(function() {
      g.clear();
      g.setFont("Vector", 20).drawString("Erasing...", 20, 80);
      fList.forEach(f => storage.erase(f));
      storage.compact();
      setupStandardReadyState();
    }, BTN1, {repeat:false, edge:"falling"});

    setWatch(function() { setupStandardReadyState(); }, BTN3, {repeat:false, edge:"falling"});
    return;
  }
...
function setupStandardReadyState() {
